...
export const ProjectCard = memo( ({ title, description, image, url, github }) => {
  ...
  return (
    ...
    <article className="relative flex justify-center w-64 p-3 px-5 m-3 h-80 rounded-md bg-neutral">
      <header>
        <img className="object-cover h-full" src={ image } alt={ title }/>
      </header>
      <footer className="absolute bottom-0 flex flex-col justify-between w-full px-1 text-center bg-gradient-to-t from-black hover:via-black hover:bg-opacity-60 h-3/5">
